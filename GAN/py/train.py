...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            x = sess.graph.get_tensor_by_name('encoder/vgg_input:0')
            y = sess.graph.get_tensor_by_name('model_2/relu4_1/Relu:0')
            ...
            for iteration in range(args.max_iter):
                ...
                # Debug
                print("Step: {}  LR: {:.7f}  Content: {:.5f}  Style: {:.5f}  TV: {:.5f}  Time: {:.5f}".format(
                    results['global_step'], results['lr'], results['content_loss'], results['style_loss'],
                    results['tv_loss'], time.time() - start))
